...
<div class="panel">
    ...
    <ul class="nav nav-tabs" id="tabAvarda">
        <li class="active">
            <a href="#avarda-overview" data-toggle="tab">{l s='Overview' mod='avardapayments'}</a>
        </li>
        <li>
            <a href="#avarda-transactions" data-toggle="tab">{l s='Transactions' mod='avardapayments'}</a>
        </li>
    </ul>
        ...
        <div class="tab-pane active" id="avarda-overview">
            <div class="row">
                <dl class="list-detail">
                    ...
                    <dd>
                        {if $avardaStatus === 1}
                            <span class="badge">{l s='Transaction authorized' mod='avardapayments'}</span>
                    ...
                    {if $avardaStatus === 1}
                        ...
                        <dd><span class="badge badge-info">{displayPrice currency=$avardaOrder->id_currency price=$avardaRemaining}</span></dd>
                        {if $avardaOrder->total_paid_tax_incl != $avardaRemaining}
                            <dt>{l s='Amount to capture' mod='avardapayments'}</dt>
                            <dd><span class="badge badge-warning">{displayPrice currency=$avardaOrder->id_currency price=$avardaOrder->total_paid_tax_incl}</span></dd>
                        {/if}
                    ...
                    {elseif $avardaStatus === 3}
                        ...
                        <dd><span class="badge badge-danger">{displayPrice currency=$avardaOrder->id_currency price=$avardaCanceled}</span></dd>
                    {elseif $avardaStatus === 2}
                        <dt>
                            {if $avardaReturned > 0}
                        ...
                        <dd><span class="badge {if $avardaReturned > 0}badge-info{else}badge-success{/if}">{displayPrice currency=$avardaOrder->id_currency price=$avardaCaptured}</span></dd>
                        {if $avardaReturned > 0}
                            <dt>{l s='Amount returned' mod='avardapayments'}</dt>
                            <dd><span class="badge badge-danger">{displayPrice currency=$avardaOrder->id_currency price=$avardaReturned}</span></dd>
                            <dt>{l s='Total amount ' mod='avardapayments'}</dt>
                            <dd><span class="badge badge-success">{displayPrice currency=$avardaOrder->id_currency price=round($avardaCaptured - $avardaReturned, 4)}</span></dd>
                        {/if}
                        {if $avardaRemaining > 0}
                            <dt>{l s='Remaining authorized amount' mod='avardapayments'}</dt>
                            <dd><span class="badge badge-warning">{displayPrice currency=$avardaOrder->id_currency price=$avardaRemaining}</span></dd>
                        {/if}
                    {/if}
                </dl>
                {if $avardaStatus === 1}
                    <div class="form form-inline">
                        ...
                        <button class="btn btn-danger" data-avarda-action="cancel">{l s='Cancel payment' mod='avardapayments'}</button>
                        <button class="btn btn-primary" data-avarda-action="capture">{l s='Capture' mod='avardapayments'}</button>
                    </div>
                    <div class="form form-inline">
                        <h4>{l s='Refund' mod='avardapayments'}</h4>
                        <input type="text" id="avarda-refund" name="avarda-refund" placeholder="{l s='Amount to refund' mod='avardapayments'}" value="{max(0, $avardaRemaining - $avardaOrder->total_paid_tax_incl)}" class="form-control fixed-width-lg">
                        <button class="btn btn-default" data-avarda-action="refund">{l s='Refund' mod='avardapayments'}</button>
                    </div>
                {elseif $avardaStatus === 2}
                    <div class="form form-inline">
                        <h4>{l s='Return item' mod='avardapayments'}</h4>
                        <input type="text" id="avarda-return" name="avarda-return" placeholder="{l s='Amount to return' mod='avardapayments'}" value="{max(0, $avardaRemaining)}" class="form-control fixed-width-lg">
                        <input type="text" id="avarda-return-reason" name="avarda-return" value="" placeholder="{l s='Reason' mod='avardapayments'}" class="form-control fixed-width-xl">
                        <button class="btn btn-default" data-avarda-action="return">{l s='Return' mod='avardapayments'}</button>
                    </div>
                {/if}
            </div>
        </div>
